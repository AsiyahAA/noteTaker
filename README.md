# noteTaker
## Description
### Project purpose was to create an application that a user can use to write and save notes using GET/POST requests, middleware, and routes. As a bonus, a delete button was to be created so that User can delete an note they wish to delete. 
## User
### User should be able to run the server in the temrinal using node.js and once server is connected, User should be able to enter in the local host plus the path name inot the searchbar to display the noteTaker html file in the borwser. User should see previous notes on left-side of the page and can enter in new notes by pressing the plus icon or refreshing the page. User can save the note by pressing the save icon to have the note saved on the left-side of the page. User should also be able to delete an old note bu pressing the trash/delete icon.
## Technologies
